...
def synthesis(model, text, speaker_ids):
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    speaker_id = 0
    text = "How are you?"
    speaker_ids = torch.Tensor([0 for _ in range(hp.speaker_size)])
    speaker_ids[speaker_id] = 1
    speaker_ids = speaker_ids.float().to(device)

    model = get_model(num)
    mel, mel_postnet, mel_postnet_torch = synthesis(model, text, speaker_ids)
